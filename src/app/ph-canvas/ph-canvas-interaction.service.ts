...
export class PhCanvasInteractionService {
  ...
  registerSheet(registration: PhCanvasSheetRegistration): void {
    ...
    if (INTERACTION_DEBUG) {
      console.log(`[PhCanvasInteraction] register ${JSON.stringify({ side: registration.side, count: this.sheets.size })}`);
    }
  }
  ...
  unregisterSheet(side: PhCanvasSideName): void {
    ...
    if (INTERACTION_DEBUG) {
      console.log(`[PhCanvasInteraction] unregister ${JSON.stringify({ side, count: this.sheets.size })}`);
    }
  }
  ...
  resolveSideAtPointer(clientX: number, clientY: number): PhCanvasSideName | null {
    ...
    // Vertical duplex overlap: prefer the sheet whose image layer is lower on screen.
  ...
  claim(side: PhCanvasSideName): void {
    if (INTERACTION_DEBUG) {
      console.log(`[PhCanvasInteraction] claim ${JSON.stringify({ from: this.activeSide, to: side })}`);
    }
  ...
  release(side: PhCanvasSideName): void {
    if (INTERACTION_DEBUG) {
      console.log(`[PhCanvasInteraction] release ${JSON.stringify({ side, activeSide: this.activeSide })}`);
    }
    if (this.activeSide === side) {
